...
func TestAppMainWithCorrectDataAndZipWithSameTopLevelDir(t *testing.T) {
	slog.SetDefault(slog.New(slog.NewTextHandler(os.Stdout, &slog.HandlerOptions{Level: slog.LevelDebug})))
	serverDir := path.Join(os.TempDir(), "omsms-server-init-test")
	defer os.RemoveAll(serverDir)
	ServerMountPath = serverDir
	t.Log("Starting with server directory: " + serverDir)
	...
	t.Setenv("OMSMS_SERVER_FILES_INIT", string(filesInitString))
	t.Setenv("OMSMS_SERVER_DEPLOYMENT_TYPE", DeploymentTypeZip)
	t.Setenv("OMSMS_SERVER_DEPLOYMENT_VALUE", "https://mediafilez.forgecdn.net/files/5972/991/Server-Files-2.1.zip")
	t.Setenv("OMSMS_SERVER_START_SCRIPT_NAME", "startserver.sh")
	...
	if _, err := os.Stat(path.Join(serverDir, "kubejs")); errors.Is(err, os.ErrNotExist) {
		t.Fatal("Expected kubejs folder to be in " + serverDir)
	}
